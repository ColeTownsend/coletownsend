...
First create a file `nano ~/.laptop.local`.

Then copy and paste the following (adjusted for your preferences:
...
# brew cask install sublime-text (this is version 2)
...
This sets you up with an install script that is run at the end of thoughtbot's [laptop](https://github.com/thoughtbot/laptop) script. Then simply follow their instructions. It's a curl command and a couple others that you should copy and paste into terminal. The `less mac` is really just a prompt and doesn't do anything you need to worry about. After quitting that prompt, run the third line. It logs everything to laptop.log as well.
...
After this you should have most of your apps install and be ready to rock and roll. Sometimes you may run into an issue when re-running this if you haven't declared an Alfred preferences file.
...
I ran this script below that changes OSX preferences, but it requires a restart. 
